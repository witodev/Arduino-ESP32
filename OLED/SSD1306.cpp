...
uint8_t font8x8_basic_tr[128][8] = { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },   // U+0000 (nul)
...
void SSD1306::defaults()
{
	tag = "SSD1306";
	ESP_LOGD(tag.c_str(), "defaults");
	SDA_PIN = GPIO_NUM_21;
	SCK_PIN = GPIO_NUM_22;
}
...
SSD1306::SSD1306(gpio_num_t SCK_PIN, gpio_num_t SDA_PIN)
{
	defaults();
...
SSD1306::SSD1306(int SCK_PIN, int SDA_PIN)
{
	defaults();

	this->SCK_PIN = (gpio_num_t) SCK_PIN;
	this->SDA_PIN = (gpio_num_t) SDA_PIN;
	init();
}

void SSD1306::init()
{
	ESP_LOGD(tag.c_str(), "init");
	init_i2c();
	init_oled();
}

void SSD1306::init_i2c()
{
	ESP_LOGD(tag.c_str(), "init_i2c");

	i2c_config_t i2c_config;

	i2c_config.mode = I2C_MODE_MASTER;
	i2c_config.sda_io_num = SDA_PIN;
	i2c_config.scl_io_num = SCK_PIN;
...
void SSD1306::init_oled()
{
	ESP_LOGD(tag.c_str(), "init_oled");
	esp_err_t espRc;
	...
	i2c_master_write_byte(cmd, OLED_CMD_SET_SEGMENT_NORMAL, true); // reverse left-right mapping
...
//	i2c_master_write_byte(cmd, OLED_CMD_DISPLAY_INVERTED, true);
	...
	i2c_master_write_byte(cmd, 0x2E, true); // stop scroll (p29)
	...
	if (espRc == ESP_OK)
	{
		ESP_LOGI(tag.c_str(), "OLED configured successfully");
	}
	else
	{
		ESP_LOGE(tag.c_str(), "OLED configuration failed. code: 0x%.2X", espRc);
	}
...
void SSD1306::print(std::string arg_text)
{
	char *text = (char*) arg_text.c_str();
	uint8_t text_len = arg_text.length();
...
void SSD1306::fillBuffer(int div)
{
	ESP_LOGD(tag.c_str(), "fillBuffer");
	int len = 128*8;
	if (data==nullptr)
		data = new uint8_t[len];
	for (int i = 0; i < len; ++i)
		data[i] = 0xFF >> (i% div);
	ESP_LOGD(tag.c_str(), "fillBuffer done");
}

void SSD1306::drawBuffer()
{
	i2c_cmd_handle_t cmd;
...
//			i2c_master_write_byte(cmd, 0xFF >> (j % 8), true);
			i2c_master_write_byte(cmd, data[i * 128 + j], true);
